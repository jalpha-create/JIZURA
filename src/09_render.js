...
class Renderer {
  ...
  frame(ctx, plan, t, opt = {}) {
    ...
    for (const P of opt.noLyrics ? [] : passes) {
      ...
      const X = LX || ctx;
      const Z = plan.centerFree && cut.zone && !cut.area ? cut.zone : null;   // a user-drawn lyric area wins over the band
      ...
      X.save();
      // a user-drawn lyric area (hirazi) wins; otherwise 中央を空ける draws the cut in its band, clipped to it
      ...
      } else if (Z) { X.beginPath(); X.rect(Z.x, Z.y, Z.w, Z.h); X.clip(); X.translate(Z.x, Z.y); }
  ...
  makeEnv(ctx, plan, cut, sc, o) {
    ...
    // lyric area (hirazi) first, then the 中央を空ける band
    const W = area ? plan.W * area.w : o.zone ? o.zone.w : plan.W, H = area ? plan.H * area.h : o.zone ? o.zone.h : plan.H;
