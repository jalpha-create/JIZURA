...
J.parseLyrics = (raw) => {
  ...
  for (let ri = 0; ri < rows.length; ri++) {
    ...
    if (!s) continue;
    const base = { text: restoreLyricEscapes(s), note, impact, emph, manual, gapBefore: pendingGap, src: ri };
    pendingGap = false;
    if (times.length) times.forEach(t => lines.push(Object.assign({}, base, { lrc: t })));
    else lines.push(Object.assign({}, base, { lrc: null }));
  }
...
J.plan = (project, audio) => {
  ...
  parsed.lines.forEach((ln, li) => {
    ...
    units.forEach((u, k) => {
      ...
      const halves = zones && !area ? splitHalf(u.text, plan.lang) : null;               // 中央を空ける: 「花が」｜「咲いた」
      ...
      const Z = area ? null : zoneOf(li), LW = area ? layoutW : Z ? Z.w : W, LH = area ? layoutH : Z ? Z.h : H;   // the frame this cut is laid out in (lyric area first)
      ...
      const cut = makeCut({ text: txt, lineText: ln.text, note: ln.note, line: li, part: k, start: cs, end: ce, layout, enter, exit, hold, inDur, outDur, params, decor, scheme: sch, seed: cutSeed, emph, recap: !!u.recap, words: J.chunkText(txt), stagger: rng.range(0.025, 0.06),
        treat, treatP, bg, bgP: bg === lineBg ? lineBgP : {}, cam, camP, trans, transP, transDur, zone: Z, area, frontmost: !!((project.lyricCutOptions || {})[`${li}:${k}`] || {}).frontmost });
      if (kime) cut.kime = true;
      if (weightGrow) cut.weightGrow = true;
      if (morph) cut.morph = morph;
      if (UU) UU.remember(li, k, txt, cut);
      if (zones && halves) splitCut(cut, halves, zones, st, dur);
  ...
  plan.cuts.forEach((c, i) => {
    c.index = i;
    if (!zones || c.zone || c.area || c.blank) return;   // lyric areas and blank cuts keep the whole frame
