...
FORK_BODY = {
    ...
    'Jevの使い方': 'Jev guide',
    'https://github.com/jalpha-create/JIZURA/blob/main/docs/JEV_GUIDE.md': 'https://github.com/jalpha-create/JIZURA/blob/main/docs/JEV_GUIDE.en.md',
    'オリジナル版（852wa/JIZURA）': 'Original edition (852wa/JIZURA)',
    '取り込み元：字面 JIZURA（hirazi fork)': 'Based on: JIZURA (hirazi fork)',
    '字面 JIZURA（jAlpha edition）': 'JIZURA (jAlpha edition)',
    '（jAlpha edition）': '(jAlpha edition)',
    'このサイトは、オリジナル版と hirazi fork を基に機能を追加した改造版です。': 'This site is a modified edition built on the original and the hirazi fork.',
    '改造版のソースコード：': 'Source code of this edition: ',
    ...
    'hirazi fork はオリジナル版を基にしたfork版です。Jevによる構成選定と、背景・前景の画像・動画編集機能などを追加しています。': 'The hirazi fork builds on the original. It adds Jev-guided arrangement and image and video editing for background and foreground layers.',
...
JALPHA_BODY = {
    ...
    "2〜5色。「背景」に選んだ色がメインの背景になり、ほかの色から文字色とアクセントを読みやすい明るさで選びます。": "2–5 colors. The color marked “Background” becomes the main background; text and accent colors are picked from the others and adjusted for readability.",
    "ロゴは全カットの上に重ねて書き出されます（透過PNG 前景／後景では前景だけ）。大きさは画面の短い辺に対する割合です。": "The logo is drawn over every cut in exports (front layer only for transparent PNG layers). Size is relative to the short side of the frame.",
    "画像を選ぶ（PNG / SVG / JPG）": "Choose image (PNG / SVG / JPG)",
    "<summary>ブランドキット ": "<summary>Brand kits ",
    ">ブランドキット</h2>": ">Brand kit</h2>",
    "aria-label=\"使うブランドキット\"": "aria-label=\"Brand kit to use\"",
    "使うブランド<select": "Brand<select",
    "＋ 新規作成": "+ New",
    "<h3>ブランド</h3>": "<h3>Brand</h3>",
    "<h3>ブランドカラー</h3>": "<h3>Brand colors</h3>",
    "＋ 色を追加": "+ Add color",
    "<h3>書体</h3>": "<h3>Fonts</h3>",
    "<h3>ロゴ</h3>": "<h3>Logo</h3>",
    "alt=\"ロゴのプレビュー\"": "alt=\"Logo preview\"",
    "ロゴを外す": "Remove logo",
    ">位置<select id=\"brandLogoPos\">": ">Position<select id=\"brandLogoPos\">",
    "<option value=\"br\">右下</option><option value=\"bl\">左下</option><option value=\"tr\">右上</option><option value=\"tl\">左上</option><option value=\"bc\">下中央</option><option value=\"tc\">上中央</option>": "<option value=\"br\">Bottom right</option><option value=\"bl\">Bottom left</option><option value=\"tr\">Top right</option><option value=\"tl\">Top left</option><option value=\"bc\">Bottom center</option><option value=\"tc\">Top center</option>",
    ">大きさ <span id=\"brandLogoSizeVal\"": ">Size <span id=\"brandLogoSizeVal\"",
    ">不透明度 <span id=\"brandLogoOpacityVal\"": ">Opacity <span id=\"brandLogoOpacityVal\"",
    "見出し<select id=\"brandFontDisplay\">": "Headline<select id=\"brandFontDisplay\">",
    "明朝枠<select id=\"brandFontSerif\">": "Serif frame<select id=\"brandFontSerif\">",
    "小さな文字<select id=\"brandFontBody\">": "Small text<select id=\"brandFontBody\">",
    "type=\"button\">キャンセル</button><button id=\"brandSave\" class=\"primary\" type=\"button\">保存</button>": "type=\"button\">Cancel</button><button id=\"brandSave\" class=\"primary\" type=\"button\">Save</button>",
    "<button id=\"brandEdit\" class=\"ghost small\" type=\"button\">編集</button><button id=\"brandDel\" class=\"ghost small\" type=\"button\">削除</button>": "<button id=\"brandEdit\" class=\"ghost small\" type=\"button\">Edit</button><button id=\"brandDel\" class=\"ghost small\" type=\"button\">Delete</button>",
...
JALPHA_UI = {
    ...
    "'ブランド：使わない'": "'Brand: off'",
    "'<option value=\"\">使わない</option>'": "'<option value=\"\">Off</option>'",
    "`ブランド「${cur.name}」を使用中：${[J.brandColors(cur).length >= 2 ? '配色' : '', Object.values(cur.fonts || {}).some(Boolean) ? '書体' : '', cur.logo ? 'ロゴ' : ''].filter(Boolean).join('・') || '（まだ中身がありません）'}をブランドの設定で固定しています。おまかせで振っても変わりません。`": "`Using brand “${cur.name}”: ${[J.brandColors(cur).length >= 2 ? 'colors' : '', Object.values(cur.fonts || {}).some(Boolean) ? 'fonts' : '', cur.logo ? 'logo' : ''].filter(Boolean).join(', ') || '(nothing set yet)'} stay fixed to the brand, even with 1-click looks.`",
    "'ブランドキットを選ぶと、配色・書体・ロゴをそのブランドの設定で固定できます。'": "'Choose a brand kit to fix colors, fonts and a logo to that brand.'",
    "`ブランド ${BR.list.length + 1}`": "`Brand ${BR.list.length + 1}`",
    "src ? 'ブランドキットを編集' : 'ブランドキットを作成'": "src ? 'Edit brand kit' : 'New brand kit'",
    "aria-label=\"ブランドカラー ${i + 1}\"": "aria-label=\"Brand color ${i + 1}\"",
    "'checked' : ''}>背景</label>": "'checked' : ''}>Background</label>",
    "title=\"この色を外す\" aria-label=\"ブランドカラー ${i + 1} を外す\"": "title=\"Remove this color\" aria-label=\"Remove brand color ${i + 1}\"",
    "'色が1つだけのときは配色を固定しません（2色以上で固定）'": "'With one color the palette is not fixed (use two or more)'",
    "'色を登録しないと、配色はスタイルのままです'": "'Without colors, the style colors are used'",
    "'<option value=\"\">スタイルの既定</option>', '<option value=\"\">指定なし</option>'": "'<option value=\"\">Style default</option>', '<option value=\"\">Not set</option>'",
    "'画像を読み込めませんでした'": "'Could not load the image'",
    "'無題のブランド'": "'Untitled brand'",
    "`ブランドキットを保存：${d.name}`": "`Saved brand kit: ${d.name}`",
    "`ブランドキット「${b.name}」を削除しますか？${inUse ? '\\nこのプロジェクトでは使うのをやめます。' : ''}`": "`Delete the brand kit “${b.name}”?${inUse ? '\\nThis project will stop using it.' : ''}`",
    "'ブランドキットがまだありません'": "'No brand kits yet'",
    "'ブランドキットのファイルではありません'": "'This is not a brand kit file'",
    "'新しいブランドキットはありませんでした（すべて登録済みです）'": "'No new brand kits (all of them are already here)'",
    "`ブランドキットを ${add.length} 件読み込みました`": "`Loaded ${add.length} brand kits`",
    "toast('ロゴ：' + (err.message || err))": "toast('Logo: ' + (err.message || err))",
    "'ブランドの配色を使用中です（ブランドを「使わない」にすると変えられます）'": "'The brand colors are in use (set Brand to Off to change them)'",
    "`${BR.list.length}件`": "`${BR.list.length}`",
    "const BRAND_POS_LABEL = { br: '右下', bl: '左下', tr: '右上', tl: '左上', bc: '下中央', tc: '上中央' };": "const BRAND_POS_LABEL = { br: 'Bottom right', bl: 'Bottom left', tr: 'Top right', tl: 'Top left', bc: 'Bottom center', tc: 'Top center' };",
