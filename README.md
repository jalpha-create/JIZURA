...
**English edition:** [Open the app](https://jalpha-create.github.io/JIZURA/en/) · [English guide](README.en.md)　／　**Bahasa Indonesia**：[Buka](https://jalpha-create.github.io/JIZURA/id/) · [Panduan](README.id.md)　**繁體中文**：[開啟](https://jalpha-create.github.io/JIZURA/zh-hant/)　**简体中文**：[打开](https://jalpha-create.github.io/JIZURA/zh-hans/)　**한국어**：[열기](https://jalpha-create.github.io/JIZURA/ko/) · [한국어 가이드](README.ko.md)
...
- **お気に入り**（jAlpha edition）：気に入った案を「☆」ボタン（再生バー）か「☆ お気に入りに追加」で残せます。サムネイル付きで一覧になり、クリックするとその見た目（スタイル・雰囲気・構成・演出・書体・配色）に戻ります。
  - お気に入りはこのブラウザに保存されるので、**別の曲にも同じ見た目を使えます**。行ごとの指定（鍵やレイアウト指定）は、歌詞が同じときだけ一緒に戻ります。
  - 名前の変更（✎）と削除（×）ができます。詳細モードの「スタイル」タブの一番上から、お気に入りをファイルに保存・読み込みできます（別のブラウザや PC へ移すとき）。最大100件です。
  - 呼び戻したあとも「◀ 前の案」で直前の見た目に戻れます。
...
## 配色・書体ライブラリ（jAlpha edition）

jcreate の配色ライブラリ（P01〜P98）と書体ライブラリ（Google Fonts の書体）を取り込んでいます。

- **配色ライブラリ**：「詳細」→「スタイル」タブの「配色ライブラリ」から選びます。選ぶとスタイルの配色を丸ごと置き換えます（文字は背景とのコントラスト比 4.5 以上、アクセントは 2.4 以上になるよう明るさを自動調整）。同じ配色をもう一度押すか「背景を切り替え」で、背景になる色が変わります。カテゴリーで絞り込めます。
- **書体ライブラリ**：「フォントの役割」の一覧に `F16 Zen Maru Gothic 900` のような名前で並びます（JIZURA にもともとある書体と同じものは除外）。欧文書体は欧文の歌詞向けです。
- **配色・書体ライブラリも使う**（おまかせの下のチェック）：オンにすると、おまかせで6割ほどの確率でライブラリの配色を使い、書体も和文のライブラリ書体が候補に入ります。かんたんモードの「ここだけ変える → 配色」もライブラリから選びます。
- ライブラリを更新したら `python tools/import_library.py` で取り込み直してから `python build.py` します（`src/11r_library.js` が作り直されます）。
- AE パネルでは、ライブラリ書体は同じ名前の書体が PC に入っていればそれを使います。

## ブランドキット（jAlpha edition）

案件ごとのブランド（配色・書体・ロゴ）をいくつでも登録しておき、プロジェクトで1つ選んで使えます。

- **作る**：「詳細」→「スタイル」タブの「ブランドキット」→「＋ 新規作成」。登録できるのは次のとおりです（どれも省略可）。
  - ブランドカラー 2〜5色（「背景」に選んだ色がメインの背景になり、ほかの色から文字色とアクセントを読みやすい明るさで選びます）
  - 書体（見出し・明朝枠・小さな文字）
  - ロゴ画像（PNG / SVG / JPG。位置・大きさ・不透明度。全カットの上に重ねて書き出されます）
- **使う**：「使うブランド」（かんたんモードでは「ブランド」）で選びます。選んでいる間は、おまかせで何度振っても配色と書体はブランドのものに固定されます。「使わない」に戻すと元の見た目に戻ります。
- ブランドキットの一覧はこのブラウザに保存されます。「ファイルに保存 / 読み込む」で別のブラウザや PC に移せます。プロジェクトファイルには使っているブランドの写しが入るので、別の PC で開いても同じ見た目になり、その PC の一覧にも自動で追加されます。
- AE 用の書き出しには、ブランドの配色と書体は入りますが、ロゴは入りません。

## タイトルカード・最後のカード（jAlpha edition）

曲の頭と終わりに、画面いっぱいのカードを重ねます。配色と書体はいまの案（ブランドキットを使っているときはブランド）に自動で合わせます。

- **タイトルカード（冒頭）**：曲名・アーティスト名・ロゴ・日付を大きく出します。オンにすると、自動で出るタイトル表示の代わりになります。文字が空欄なら曲名・アーティスト名を使います。
- **最後のカード（CTA）**：「配信中」「プロフィールのリンクから」などの呼びかけと、**QR コード**・ロゴ・日付を最後の数秒に出します。横長では文字の右に、縦長では下に QR コードを置きます。QR コードは読み取りやすいよう、いつも白地に黒で描きます。
- 設定は「詳細」→「カード」タブ（かんたんモードではオン・オフと、最後の文字・QR の URL）。ロゴは「ブランドキットのロゴ」か、カードごとに画像を選べます。日付は「今日」ボタンで入れられます。
- プレビューにも書き出し（MP4・連番PNG・まとめて書き出し）にも入ります。透過PNG では背景を塗らず、前景／後景の書き出しでは前景に入ります。AE 用の書き出しには入りません。
- QR コードは同梱の qrcode-generator（MIT）で作ります。外部のサーバーには何も送りません。公開前に、スマホで読み取れるか確かめてください。
...
- **歌詞を消す／初期化**：「歌詞を消す」（歌詞欄の右上）は、歌詞と、行ごとのタイミング・指定・書き出す範囲を消します。「元に戻す」か Ctrl+Z で戻せます。ヘッダーの「初期化」は、歌詞・曲（このブラウザに残していた分も）・設定・案の履歴・取り消しの履歴をすべて消して最初の状態に戻します（確認のあと実行。元に戻せません）。表示モード・音量・画面の言語はそのままです。
   ...
   - **まとめて書き出し**（jAlpha edition）：書き出す画面比を複数選び（初期値は 16:9・9:16・1:1）、ボタン1つで MP4 を1本ずつ順番に保存します。構成は画面比ごとに同じシードで組み直すので、縦長では縦長向けの配置になります。ファイル名は `曲名_16x9.mp4` のように画面比が付きます。解像度・fps・画質・背景・曲の有無は今の設定のまま使います。Chrome では2本目の保存時に「複数ファイルのダウンロード」の許可を聞かれるので、許可してください。
...
再生バーの「セーフエリア」で、縦型 SNS のアプリ画面に隠れる範囲をプレビューに重ねて表示します（書き出しには入りません。選んだ表示はブラウザが覚えます）。

- **TikTok / Instagram リール / YouTube ショート**：右側のボタン列・下部のキャプション欄・上部のタブで隠れる範囲を薄い赤で塗り、点線の内側（セーフエリア）を示します。「SNS 3つを重ねる」は3つのいちばん狭い範囲です。9:16 用の枠なので、ほかの画面比ではその旨を表示します。
- 数値は 1080×1920 での目安です（TikTok：上160・右150・下480・左60px、リール：上220・右140・下420・左60px、ショート：上190・右190・下440・左60px）。各アプリの画面は変わることがあるので、公開前に実機でも確認してください。
- **放送用セーフ（全画面比）**：アクションセーフ 93%・タイトルセーフ 90% の枠です。
...
1. [`JIZURA_CEP.zip`](https://jalpha-create.github.io/JIZURA/JIZURA_CEP.zip) をダウンロードして展開します。
